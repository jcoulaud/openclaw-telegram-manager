...
describe('setup integration', () => {
  ...
  describe('uninstall', () => {
    ...
    it('should restore inline groups from JSON5 include file on uninstall', () => {
      const configPath = path.join(configDir, 'openclaw.json');
      const includePath = path.join(configDir, 'telegram-manager.generated.groups.json5');
      ...
      // Include file written by generateInclude — JSON5 format with topics
      const includeObj = {
        '-1003731538650': {
          ...
          topics: {
            '123': {
              enabled: true,
              skills: ['coding-agent'],
              systemPrompt: 'You are the assistant for the Telegram topic: My Project.\n\nDeterminism rules:\n- Source of truth...',
            },
          },
        },
        '-1009876543210': {
      ...
      const fileContent = [
        '// This file is generated by telegram-manager. Do not hand-edit.',
        '// Rebuild from: /home/user/.openclaw/workspace/projects/topics.json',
        '// registry-hash: sha256:abc123',
        json5Content,
        '',
      ].join('\n');
      fs.writeFileSync(includePath, fileContent, { mode: 0o600 });

      // Simulate unpatchConfig with JSON5 parsing + topics stripping
      const content = fs.readFileSync(configPath, 'utf-8');
      const config = JSON.parse(content);
      const telegram = config.channels?.telegram;
      if (telegram && fs.existsSync(includePath)) {
      ...
      const restored = JSON.parse(fs.readFileSync(configPath, 'utf-8'));
      expect(restored.channels.telegram.token).toBe('secret-token');
      // Groups restored with only group-level settings, no topics
      expect(restored.channels.telegram.groups).toEqual({
        '-1003731538650': { requireMention: false },
        '-1009876543210': { requireMention: true },
      });
      expect(fs.existsSync(includePath)).toBe(false);
  ...
  describe('patchMemoryFlush', () => {
    ...
    function patchMemoryFlush(cfgDir: string): void {
      ...
      if (raw.includes(MEMORY_FLUSH_INSTRUCTION)) return;
    ...
    it('should be idempotent (does not duplicate)', () => {
      ...
      const occurrences = prompt.split(FLUSH_TAG).length - 1;
    ...
    it('should update instruction when wording changes but tag stays', () => {
      ...
      config.agents = {
        defaults: {
          compaction: {
            memoryFlush: { prompt: oldTaggedInstruction },
          },
        },
      };
      fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + '\n');
